...
#print(table_1)
        ...
        excel_file_name = specified_location+'/'+gstin+'_'+month+'.xlsx'
    ...
    if request.method == "POST" :
        ...
        if os.path.exists(path):
            if os.path.isfile(path):
                result = 'The Pdf File Has Been Converted To CSV'
                error,directory = create_directory()
                
                excel_file_url,link,_ = pdf_converter(path,directory)
            else :
                filenames = scan_directory(path)
                result = pdf_files_validator(filenames)
                if len(filenames) == 12 :
                    if result == 'The Pdf File Has Been Converted To CSV' :
                        error,directory = create_directory()
                        dataframes_list = []
                        for i in filenames:
                            excel_file_url,link,x = pdf_converter(i,directory)
                            dataframes_list.append(x)
                
                        consolidated_sheet_generator(dataframes_list,directory)
                        #result='The Pdf File Has Been Converted To CSV'
            ...
            excel_file_url = ''
            link = ''
        return render_template("index.html", res=result,url = excel_file_url,link = link)
        #return 'abc'
    else :
         return "404 error"
