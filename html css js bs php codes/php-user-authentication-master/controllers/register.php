<?php
    ...
    if(isset($_POST["submit"])) {
        ...
        $table_data = mysqli_query($connection, $data);
        $rowCountData = mysqli_num_rows($table_data);
            ...
            if($rowCount > 0) {
                ...
                if((preg_match("/^[a-zA-Z ]*$/", $_first_name)) && (preg_match("/^[a-zA-Z ]*$/", $_last_name)) &&
                 ...
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    // Query
                    $sql = "INSERT INTO users (id,firstname, lastname, email, mobilenumber, password, token, is_active,
                    date_time) VALUES ('{$id}'{$firstname}', '{$lastname}', '{$email}', '{$mobilenumber}', '{$password_hash}', 
                    '{$token}', '1', now())";
                    ...
                    if($sqlQuery) {
                        $email_verify_success = '<div class="alert alert-success">profile has been registered!</div>';
                    }
                }
            }
        } else {
